...
from logic.logic_ui.filter_page import FilterPage  # Renamed to reflect new focus
    ...
    def test_run_grid_parallel(self):
        if self.browser.grid_enabled and not self.browser.serial_enabled:
            with concurrent.futures.ThreadPoolExecutor(max_workers=len(self.browser.browser_types)) as executor:
                executor.map(self.test_fill_filter_search_and_reset, self.browser.browser_types)

        else:
            self.test_fill_filter_search_and_reset(self.browser.default_browser)
